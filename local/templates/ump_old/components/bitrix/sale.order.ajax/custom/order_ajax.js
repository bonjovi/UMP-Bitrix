...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		refreshOrder: function(result)
		{
			...
			var person_type = $('input[name="PERSON_TYPE"]:checked').parent().text();
			
			if(person_type == 'Юридическое лицо') {
				//$('<div class="found_dealers">Мы нашли дилеров в вашем городе, где Вы можете приобрести нашу продукцию</div>').appendTo('#bx-soa-region');
				var currentCity = $('body').find('.bx-ui-sls-fake').val();
				$.ajax({
					type: "GET",
					url: '/local/templates/ump/find_dealers.php',
